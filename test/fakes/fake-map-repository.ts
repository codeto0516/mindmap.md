...
/**
 * Application 単体テスト用の MapRepository インメモリ実装。
 * Domain のみに依存し、Infrastructure を参照しない。
 */
export function createFakeMapRepository(): MapRepository & {
  ...
  return {
    ...
    async findByWorkspaceAndFolder(
      ...
      return Array.from(maps.values())
        .filter((m) => m.workspaceId === workspaceId && m.folderId === folderId)
        .map((m) => ({ ...m }))
